...
public class TicketUtils {
	...
	private static final int ut_timeout = 24 * 60 * 60; // 分钟
	private static final long COOKIE_LIFE = 1;
	...
	public static UserCookie generateOpenIdTicket(UserCache userCache,
		...
		try {
			...
			if (null != req && null != resp) {
				...
				String domain = SsoClientUtil.getDomain();// TODO dusai
	...
	public static UserCookie generateTicket(UserCache userCache,
		...
		try {
			...
			if (null != req && null != resp) {
				...
				Long cookieLife = COOKIE_LIFE;
				if (cookieLife != null && userCache.isAutoLogin()) {// 自动登录cookie保存一天,否则关闭浏览器,cookie失效
	...
	public static UserCache getUser(CacheProxy proxy, String ut) {
		...
		try {
			...
			// 时间计算错误, 乘法优于除法.
			long remain = ut_timeout * 1000 * 60 - (now - last); // 缓存剩余分钟数
			if (remain <= 0) {
				return null;
			}
			if (remain < 600000) {
	...
	public static boolean setUnionOpenId(UserUnionLoginDTO userUnionLoginDTO,
		...
		if (!StringUtils.isBlank(ut)
				&& !StringUtils.isBlank(userUnionLoginDTO.getOpenid())) {
