...
$(document).ready(function() {
    ...
    /**覆盖原因：
     *      项目中页面是用angularjs控制页面数据
     *      数据一般都是ajax从后台拿回数据后再绑到页面上，页面刚加载完img src属性值都是空的
     *      此插件页面加载完后执行，这时img src还没有值。点击图片不会出现遮罩放大效果
     * 修改思路：
     *      定时器处理，不断的找图片src是否有值。img里有图片地址才绑定灯箱。
     * */
    var deal = new Array();
    var intervalProcess = setInterval(function(){
        $('#gallery img').each(function(i){
            //console.info("图片灯箱定时器在执行！");
            if(this.src && !deal[i]){
                $(this).fsgallery();
                deal[i] = true;
            }
            if(deal.length >= $('#gallery img').length){
            	//$('#gallery img').fsgallery();
