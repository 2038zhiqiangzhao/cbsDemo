...
angular.module('tm.pagination', []).directive('tmPagination',[function(){
    return {
        restrict: 'EA',
        template: '<div><ul class="pagination" ng-show="conf.numberOfPages > 0">'+
                    '<li class="size">显示：<select ng-model="conf.itemsPerPage" ng-change="changeItemsPerPage()" ng-options="ppo as ppo for ppo in conf.perPageOptions"></select> 条，共{{ conf.totalItems }}条</li>'+
                    '<li class="fenye">'+
                      '<a href="javascript:void(0)" ng-class="{disabled: conf.currentPage == 1}" ng-click="prevPage()">&lt;上一页</a>'+
                      '<a href="javascript:void(0)" ng-repeat="item in pageList track by $index" ng-class="{active: item == conf.currentPage, omit: item == \'...\'}" ng-click="changeCurrentPage(item)">{{ item }}</a>'+
                      '<a href="javascript:void(0)" ng-class="{disabled: conf.currentPage == conf.numberOfPages}" ng-click="nextPage()">下一页&gt;</a>'+
                    '</li>'+
                    '<li class="index">共{{numberOfPages}}页，到第 <input class="num" type="text" ng-model="jumpPageNum"> 页 <button type="button" ng-click="jumpToPage()" class="btn">确定</button></li>'+
                  '</ul><div ng-if="conf.totalItems == 0">暂无数据</div></div>',
        replace: true,
        scope: {
            conf: '='
        },
...
//                alert((nv[0]+nv[2]) == (ov[0]+ov[2]));
                if(scope.conf.onChange && (nv[0]+nv[2]) != (ov[0]+ov[2])){
