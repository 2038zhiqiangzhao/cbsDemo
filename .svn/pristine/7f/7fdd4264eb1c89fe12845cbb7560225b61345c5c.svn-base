...
/**
 * 
 * 项目名称：morning-common-base 类名称：DateUtils 类描述：工具类：提供一些日期通用处理的方法 创建人：陈星星
 * 创建时间：2017年2月5日 下午10:06:44
 */
public class DateUtils {
	...
	/** 标准日期时间格式化器 */
	private static final String NORM_DATETIME_FORMAT = new String(
			"yyyy-MM-dd HH:mm:ss");
	...
	public static Date parseDateTime(String dateString) {
		try {
			...
			logger.error("Parse " + dateString + " with format "
					+ new SimpleDateFormat(NORM_DATETIME_FORMAT).toPattern()
					+ " error!", e);
		}
		return null;
	}
	...
	public static Date parseDate(String dateString) {
		try {
			...
			logger.error("Parse " + dateString + " with format "
					+ new SimpleDateFormat(NORM_DATE_FORMAT).toPattern()
					+ " error!", e);
		}
		return null;
	}
	...
	public static String dateDiff(Date subtrahend, Date minuend) {
		...
		if (diff <= HOUR_MS)
			...
			return diff / DAY_MS + "天前" + calendar.get(Calendar.HOUR_OF_DAY)
					+ "点" + calendar.get(Calendar.MINUTE) + "分";
		else if (diff <= YEAR_MS)
			return diff / MONTH_MS + "个月" + (diff % MONTH_MS) / DAY_MS + "天前"
					+ calendar.get(Calendar.HOUR_OF_DAY) + "点"
					+ calendar.get(Calendar.MINUTE) + "分";
		else
			return diff / YEAR_MS + "年前" + (calendar.get(Calendar.MONTH) + 1)
