...
public class CacheCommUtils {
	...
	private static synchronized void initCache() {
		 ...
		 * CLIENT_FILE_PATH); } //File clientFile=new
		 * File(System.getProperty("global.config.path") , CLIENT_FILE_PATH);
		 * 
		 * //logger.info("配置文件地址=" + dir); //File file = new File(dir); if
		 * (clientFile.exists()) { cache = MemcacheExtend.getInstance("file:" +
		 * clientFile.getAbsolutePath()).getCacheProxy("user"); if (cache ==
		 * null) { throw new
		 * RuntimeException("sso client memcache loading fail, memcacheFileDir is:"
		 * + clientFile.getAbsolutePath()); } } else { throw new
		 * RuntimeException(clientFile.getAbsolutePath() + " is not exist."); }
		 * }
		 */
		// cache = new LocalCacheStorage();// 本地缓存
