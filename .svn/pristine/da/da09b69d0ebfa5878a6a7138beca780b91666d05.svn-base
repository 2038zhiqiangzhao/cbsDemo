...
public class FilterUtil {
	...
	public static String getProperty(String key) {
		...
		String value = "";
		/*
		 * InputStream is=null; try { is = new BufferedInputStream(new
		 * FileInputStream(clientFile)); Properties properties = new
		 * Properties(); properties.load(is); value =
		 * properties.get(key).toString(); } catch (Exception e) {
		 * LogUtils.getLogger(FilterUtil.class).error(e.getMessage(),e);;
		 * }finally{ if(null!=is){ try { is.close(); } catch (IOException e) {
		 * LogUtils.getLogger(FilterUtil.class).error(e.getMessage(),e); } } }
		 */
		return value;
	}
