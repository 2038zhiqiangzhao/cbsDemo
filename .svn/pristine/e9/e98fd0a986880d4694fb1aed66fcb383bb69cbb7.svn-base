...
/**
 * 
 * 项目名称：morning-common-base 类名称：StringUtil 类描述：工具类：提供一些字符串操作的方法 创建人：陈星星
 * 创建时间：2017年1月18日 下午3:44:33
 * 
 * @version
 */
...
/** HTML字符转义:"<" */
	...
	/**
	 * 除去字符串头尾部的空白，如果字符串是null,依然返回null 注意:和<code>String.trim</code>不同，此方法使用
	...
	/**
	 * 获得set或get方法对应的标准属性名 例如：setName 返回 name
	 * 
	...
	public static boolean isSurround(CharSequence str, char prefix, char suffix) {
		...
		return str.charAt(0) == prefix
				&& str.charAt(str.length() - 1) == suffix;
	}
	...
	public static String format(String template, Object... params) {
		...
		return strFormatter(template, params);
	}
	...
	public static byte[] bytes(String str, String charset) {
		return bytes(
				str,
				isBlank(charset) ? Charset.defaultCharset() : Charset
						.forName(charset));
	}
	...
	public static int count(final String content, final String strForSearch) {
		if (hasEmpty(content, strForSearch)
				|| strForSearch.length() > content.length()) {
			return 0;
		}
	 ...
	 * @return 结果
	 */
	public static String strFormatter(final String strPattern,
		...
		for (int argIndex = 0; argIndex < argArray.length; argIndex++) {
			...
			if (delimIndex == -1) {// 剩余部分无占位符
				...
				if (delimIndex > 0
						&& strPattern.charAt(delimIndex - 1) == C_BACKSLASH) {// 转义符
					if (delimIndex > 1
							&& strPattern.charAt(delimIndex - 2) == C_BACKSLASH) {// 双转义符
