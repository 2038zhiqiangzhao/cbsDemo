...
import com.people2000.common.cache.IRemoteCacheStore;
...
/**
 * 通过redis存储缓存数据 1.支持序列化json
 * 
 * <p style="display:none">
 * modifyRecord
 * </p>
 * <p style="display:none">
 * version:V1.0,author:ningyu,date:2012-10-30 下午5:07:21,content:
 * </p>
 * 
 * @author ningyu
...
public class RedisCacheStorage<K, V> implements IRemoteCacheStore<K, V>,
	...
	public void removeMulti(K... keys) {
		...
		if (keys == null) {
			throw new RedisCacheStorageException(
					"keys does not allow for null!");
		}
		Jedis j = null;
	...
	public V hget(String cacheId, K key) {
		...
		if (StringUtils.isBlank(cacheId)) {
			throw new RedisCacheStorageException(
					"cacheId does not allow for null!");
		}
		Jedis j = null;
		String skey = CacheUtils.toJsonString(key);
		if (skey == null) {
		...
		try {
			...
			if (map != null) {
				for (Map.Entry<String, String> entry : map.entrySet()) {
					...
					result.put((K) CacheUtils.jsonParseObject(entry.getKey()),
							(V) CacheUtils.jsonParseObject(entry.getValue()));
				}
				return result;
			}
		} catch (JedisException e) {
	...
	private void retryStorage(String cacheId, Map<K, V> map) {
		if (strongTask == null) {
			strongTask = new StrongCacheTask("重试初始化Strong Cache任务", cacheId,
					map);
	 ...
	 * 具体环境情况
	 * </p>
	 * 1.获取storage缓存初始化状态 2.需要初始化时锁定并初始化 3.如果锁定不成功，稍后再试
	 * 
	 * @author ningyu
	...
	public void initializationStrongCache(final String cacheId, Map<K, V> map) {
		...
		try {
			j = client.getResource();
			String isSuccess = j.get("DPAP.redis.strong.initialization."
					+ cacheId);
			if (StringUtils.isBlank(isSuccess) || isSuccess.equals("false")) {// 如果对应的cacheId
																				// 没有初始化过或者初始化失败
				boolean lock = client.getLock(j, "DPAP.redis.lock." + cacheId);
				if (lock) {
					for (Map.Entry<K, V> entry : map.entrySet()) {
						String key = CacheUtils.toJsonString(entry.getKey());
						String value = CacheUtils
								.toJsonString(entry.getValue());
						if (key == null || value == null) {
							log.error("storage cache initialization error: key and value does not allow for null!");
							j.set("DPAP.redis.strong.initialization." + cacheId,
									"false");
							client.releaseLock(j, "DPAP.redis.lock." + cacheId);
							return;
						}
						j.hset(cacheId, key, value);
					}
					j.set("DPAP.redis.strong.initialization." + cacheId, "true");
					client.releaseLock(j, "DPAP.redis.lock." + cacheId);
				} else {
					// 稍后重试
	 ...
	 * </p>
	 * <p style="display:none">
	 * version:V1.0,author:ningyu,date:2012-10-22 下午1:43:11
	 * </p>
	 * 
	 * @author ningyu
		...
		public void run() {
			...
			while (true) {
				...
				try {
					j = client.getResource();
					String isSuccess = j
							.get("DPAP.redis.strong.initialization." + cacheId);
					if (StringUtils.isBlank(isSuccess)
							|| isSuccess.equals("false")) {// 如果对应的cacheId
															// 没有初始化过或者初始化失败
						boolean lock = client.getLock(j, "DPAP.redis.lock."
								+ cacheId);
						if (lock) {
							for (Map.Entry<K, V> entry : map.entrySet()) {
								String key = CacheUtils.toJsonString(entry
										.getKey());
								String value = CacheUtils.toJsonString(entry
										.getValue());
								if (key == null || value == null) {
									log.error("storage cache initialization error: key and value does not allow for null!");
									j.setnx("DPAP.redis.strong.initialization."
											+ cacheId, "false");
									client.releaseLock(j, "DPAP.redis.lock."
											+ cacheId);
									return;
								}
