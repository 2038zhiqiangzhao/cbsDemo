...
public class OSSClientUtilTest {
	...
	// 阿里云API的bucket名称
	public static final String BACKET_NAME = "medical-park-testing-environment";
	// 阿里云API的密钥Access Key ID
	public static final String ACCESS_KEY_ID = "LTAIm6UxVIQ8ggQF";
	// 阿里云API的密钥Access Key Secret
	public static final String ACCESS_KEY_SECRET = "VcbsdhFsmSwfOG1fevS4ywZpAFrRXft";
	// 阿里云API的文件夹名称
	public static final String FOLDER = "cbs/";

	private static Logger logger = Logger.getLogger(OSSClientUtilTest.class);
	...
	public static String upload(File file) {
		...
		try {
			...
			metadata.setContentDisposition("filename/filesize=" + fileName
					+ "/" + fileSize + "Byte.");
			// 上传文件 (上传文件流的形式)
			PutObjectResult putResult = getOSSClient().putObject(BACKET_NAME,
					FOLDER + fileName, is, metadata);
			// 解析结果
	...
	public static String getUrl(String key) {
		...
		Date expiration = new Date(System.currentTimeMillis() + 3600L * 1000
				* 24 * 365 * 10);
		// 生成URL
		URL url = OSSClientUtilTest.getOSSClient().generatePresignedUrl(
				BACKET_NAME, key, expiration);
		if (url != null) {
			return url.toString();
		}
		return null;
	...
	public static void main(String[] args) {
		// 上传文件
		String files = "D:\\image\\sxlkj20060214085.jpg";
		String[] file = files.split(",");
		for (String filename : file) {
			System.out.println("filename:" + filename);

			//File file1 = new File(filename);
			//String md5key = OSSClientUtil.upload(file1);
			// 上传后的文件MD5数字唯一签名:40F4131427068E08451D37F02021473A
			//System.out.println("上传后的文件MD5数字唯一签名:" + md5key);

			// OSSObject oss = getOSSClient().getObject(BACKET_NAME, md5key);
			// System.out.println(oss.getObjectContent());
			Date expiration = new Date(new Date().getTime() + 3600l * 1000 * 24
					* 365 * 10);
			URL url = getOSSClient().generatePresignedUrl(BACKET_NAME, "6EA805FD64A40C593A2473250E6A5D94",
					expiration);
			if (url != null) {
