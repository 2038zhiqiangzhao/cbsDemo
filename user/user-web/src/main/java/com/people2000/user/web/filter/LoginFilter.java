 ...
 *
 */
package com.people2000.user.web.filter;
...
import com.people2000.user.business.utils.FilterUtil;
import com.people2000.user.business.utils.RequestUtils;
import com.people2000.user.business.utils.ResponseUtil;
...
public class LoginFilter implements Filter {
	...
	private UserCache getUserCache(HttpServletRequest req) {
		...
		// TODO 从缓存取的过程本身应该在getUserByLoginCookie内部包含了，存在意义不大，暂时和原始代码保持一致
		UserCache userCache = SsoClientUtil.getUser(ut);
		if (userCache == null) {
			/*
			 * try { userCache =
			 * UserLoginServiceClient.getInstanst().getUserByLoginCookie(ut); }
			 * catch (Exception e) { logger.error("查询用户信息异常，ut=" + ut, e); }
			 */
		}
		return userCache;
	}
